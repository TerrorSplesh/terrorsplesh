...
with col1:
    input_url = st.text_input("🔗 Hawk Live Match URL", placeholder="https://hawk.live/dota-2/matches/...", key="url_input")
    if input_url:
        st.session_state.match_url = input_url
...
if st.session_state.match_url:
    match_url = st.session_state.match_url
    ...
    if teams and len(teams) >= 2:
        ...
        st.markdown(f'<p class="last-update">🕐 Обновлено: {st.session_state.last_update}</p>', unsafe_allow_html=True)
        
        st.divider()
        st.info("💡 Нажми 'Обновить' для получения актуальных коэффициентов")
        
    else:
        st.error(f"Не удалось распознать команды: {tournament}")
else:
    st.info("👆 Введите ссылку на матч с Hawk Live")
